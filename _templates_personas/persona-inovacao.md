...
[LISTAR componentes críticos que NÃO podem ser alterados]
- API proxy route — injeta auth automaticamente
- Hooks de Realtime — fonte de dados ao vivo
- WebSocket connections — reconexão já funciona

# Ambientes
- Trabalhar APENAS na branch [BRANCH]
- Frontend staging: [URL/MÉTODO] (auto-deploy)
- Frontend produção: [URL/MÉTODO] (auto-deploy)
- **NAO COMMITAR** — MauMau revisa e commita. Salvar mudancas e reportar.
- Rodar `npm run build` após cada task
